  ...
  return (
    <XStack ai="center" jc="center" mt={12} gap={8}>
      <Button
        ...
        size="$1"
        circular
        onPress={() => updateBear("add")}
        {...rest}
      >
        <PlusCircle />
      </Button>
      <Circle bg="$red10" size="$2" borderWidth={2}>
        <Text>{bearsList.length}</Text>
      </Circle>

      <Button
        pressStyle={{ bg: "$red8" }}
        disabled={bearsList.length === 0}
        bg={bearsList.length !== 0 ? "$red9" : "$gray10"}
        size="$1"
        circular
        onPress={() => updateBear("remove")}
        {...rest}
      >
        <MinusCircle />
      </Button>
